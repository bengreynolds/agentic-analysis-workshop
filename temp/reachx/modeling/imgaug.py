"""
imgaug.py - Implementation of the "imgaug" dataset format


--------------------------------------
CREDIT: The code in this module is excerpted and adapted from the DeepLabCut toolbox repository:
...
_MAT_FILE_FOR_TRAINING = "train.mat"
...
class ImgaugPoseDataset:
    def __init__(self, cfg):
        self.cfg = cfg
        self._n_kpts = len(cfg["all_joints_names"])
        self.has_gt = False  # set by load_dataset()
        self.raw_data = {}   # ditto
        ...
        if cfg.get("rotation", True):
            ...
            if isinstance(opt, int):
                ...
                cfg["rotation"] = 25
            cfg["rotratio"] = cfg.get("rotratio", 0.4)  # fraction of training samples with rotation augmentation
        else:
            cfg["rotratio"] = 0.0
            cfg["rotation"] = 0
    ...
    def load_dataset(self):
        # in ReachX, we use the MAT file rather than the pickle file to load the training dataset
        cfg = self.cfg
        file_name = str(os.path.join(self.cfg["train_dir"], _MAT_FILE_FOR_TRAINING))
        ...
        for i in range(num_images):
            ...
            item = DataItem()
            item.image_id = i
            # NOTE: In ReachX implementation, we just store the image file name. We need to create absolute
            # path to image files: <train_dir>/images/<img_file_name>.
            img_file_name = sample[0][0]
            im_path = Path(cfg["train_dir"], "images", img_file_name)
            item.im_path = str(im_path.absolute())
            item.im_size = sample[1][0]
            if len(sample) >= 3:
                joints: np.ndarray = sample[2][0][0]
                joint_id = joints[:, 0]
    ...
    def build_augmentation_pipeline(self, height=None, width=None, apply_prob=0.5):
        ...
        def sometimes(augment_op):
            return iaa.Sometimes(apply_prob, augment_op)
        ...
        # ReachX does not use "fliplr" or "symmetric_pairs", so this augmentation omitted
        #
        ...
        # ReachX does not use the "gaussian_noise" or "grayscale" augmentations
    ...
    def get_batch(self):
        ...
        # Scale is sampled only once to transform all the images of a batch into same size.
        ...
        for i in range(self.batch_size):
            ...
            # read in image using CV2 and convert from CV2's native BGR format to RGB
            image = cv2.imread(data_item.im_path, cv2.IMREAD_UNCHANGED)[..., ::-1]
    ...
    def get_scmap_update(self, joint_ids, joints, data_items, sm_size, target_size):
        ...
        for i in range(len(data_items)):
            ...
            (
                ...
                locref_target,
                locref_mask,
            ) = self.compute_target_part_scoremap_numpy(joint_ids[i], [joints[i]], sm_size, scale)
    ...
    def compute_target_part_scoremap_numpy(self, joint_id, coords, size, scale):
